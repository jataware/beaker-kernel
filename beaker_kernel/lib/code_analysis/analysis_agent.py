...
# code_start_pos: int = Field(description="Offset where annotation starts in bytes if code is treated as a bytestring.")
# code_end_pos: int = Field(description="Offset where annotation ends in bytes if code is treated as a bytestring.")
...
#     def set_annotation_types(self, annotation_types: list[AnalysisCategory]):
#         content = [
#             f"""\
# Below are the types of annotations/issues that you should use when analyzing code and generating analyses.
# All generated analyses should match exactly one of these rules.
...
#             for annotation in annotation_type.issues:
#                 f"""\
...
#         self.add_context("\n\n".join(content))
    ...
    def code_analysis(self, analysis_list: list[AnalysisResult], loop: LoopControllerRef) -> list[AnalysisResult]:
        ...
        #     result.append(AnalysisAnnotation(
