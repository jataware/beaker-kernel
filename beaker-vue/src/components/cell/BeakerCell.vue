<template>
    ...
    >
        ...
        <div class="state-contents">
            <div class="execution-count execution-count-badge horizontal-center"
                :class="[(isBusy ? 'busy' : null), cellState]"
            >
            ...
            <Dropdown class="cell-type-selector" v-model="cell_type" :options="Object.keys(props.availableCellTypes)">
                <template #value="slotProps">
                    <img :src="cellIconMap[slotProps.value]"/>
                </template>
                <template #option="slotProps">
                    <div class="cell-type-dropdown-item">
                        <img :src="cellIconMap[slotProps.option]"/> <span>{{ slotProps.option }}</span>
                    </div>
                </template>
            </Dropdown>
        </div>
...
import { defineProps, ref, computed, defineEmits, defineExpose, Component, nextTick, withDefaults } from "vue";
...
// TODO: Include/allow both dark and light versions.
const cellIconMap = {
...
export interface Props {
    cell: IBeakerCell;
    availableCellTypes: {[key: string]: Component};
    dragEnabled?: boolean;
}
...
const cellComponent = computed((): IBeakerCellComponent => {
...
const cellStateChanged = (newState: string) => {
