<template>
  <Tree :value="contextNodes" :loading="!props.context" class="context-tree" v-model:expandedKeys="contextExpandedKeys">
    ...
    <template #action="slotProps">
      <div @mousedown="($event.detail > 1) && $event.preventDefault();"
        @dblclick.stop.prevent="selectAction(slotProps.node.label)"
        style="cursor: pointer; border-bottom: 1px dotted var(--text-color-secondary);" v-tooltip="{
    ...
    <template #tool="slotProps">
      <span style="cursor: help; border-bottom: 1px dotted var(--text-color-secondary);" v-tooltip="{
        value: `${slotProps.node.data}`,
        pt: {
          text: {
            style: {
