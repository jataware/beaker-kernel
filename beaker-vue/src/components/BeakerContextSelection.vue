<template>
  <Dialog class="context-selection-dialog" v-bind:visible="props.isOpen" @update:visible="closeDialog" closable modal
    header="Configure Context">
    <p>
      Select context and language.
    </p>
    <InputGroup>
      <Dropdown v-model="selectedContextSlug" :options="contextOptions" optionLabel="slug" optionValue="slug" />

      <Dropdown v-model="selectedLanguage" :options="languageOptions" optionLabel="slug" optionValue="subkernel" />
    </InputGroup>

    <h4 class="h-less-pad">Context Info</h4>

    <div class="code-container">
      <Codemirror :extensions="codeExtensions" :tab-size="2" language="javascript"
    ...
    <template #footer>
      <div style="width: 100%; text-align: center;">
        <Button :loading="props.contextProcessing" raised @click="setContext" label="Apply" size="small" />
      </div>
    </template>
