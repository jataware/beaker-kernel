<template>
            ...
            >
                ...
                <SideMenuPanel
                    v-if="config.config_type !== 'server'"
                    id="config"
...
// console.log({"this": });
export const leftSidePanel = <SideMenu
                ref="sideMenuRef"
                position="left"
                highlight="line"
                expanded={true}
                initialWidth="25vi"
                maximized={instance?.isMaximized.value}
            >
                <SideMenuPanel label="Context Info" icon="pi pi-home">
                    <InfoPanel/>
                </SideMenuPanel>
                <SideMenuPanel id="files" label="Files" icon="pi pi-folder" no-overflow lazy={true}>
                    <FilePanel
                        ref="filePanelRef"
                        open-file={instance.loadNotebook}
                        preview-file={(file, mimetype) => {
                            const previewedFile = {url: file, mimetype: mimetype};
                            const previewVisible = true;
                            {/* rightSideMenuRef.value.selectPanel('file-contents'); */}
                        }}
                    />
                </SideMenuPanel>
                {/* <SideMenuPanel icon="pi pi-comments" label="Chat History">
